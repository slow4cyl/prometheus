...
A retired scheduled writer repeatedly forced unrelated rows into adversarial_ml, nlp, and optimization
(the attack->adversarial_ml, cross_lingual->nlp, distillation->optimization
June-8 lossy merges — the three targets the four-bucket pass did not cover).  The earlier one-shot repaired
...
BUCKETS = frozenset({"adversarial_ml", "nlp", "optimization"})
CONFIDENCE = 0.50
MARGIN = 0.15
BATCH_SIZE = 256
UNDO_TABLE = "lossy_domain_backfill_undo_mergetargets_v1"
...
def collect_candidates(conn, buckets: Iterable[str] = BUCKETS) -> list[dict]:
    ...
    for row in conn.execute(
        ...
        SELECT 'experiments' AS table_name, id AS row_id, domain,
               TRIM(SUBSTR(COALESCE(hypothesis, ''), 1, 300) || ' ' ||
                    SUBSTR(COALESCE(result, ''), 1, 300)) AS text
          FROM experiments
    ...
    for row in conn.execute(
        ...
        SELECT 'worker_results' AS table_name, wr.id AS row_id, wr.domain,
               TRIM(SUBSTR(COALESCE(e.hypothesis, ''), 1, 300) || ' ' ||
                    SUBSTR(COALESCE(wr.finding, ''), 1, 300)) AS text
