...
def apply_results():
    ...
    with get_db() as conn:
        ...
        with state_write_lock() as state:
            ...
            for row in results:
                (result_id, exp_id, task_id, hyp_supported, key_finding,
                 confidence, domain, tags_json, files_json, queue_json,
                 worker_id, created_at, pred_dir_json, obs_dir_json, design_vec_json,
                 experiment_type, mechanism_type, wr_model, calibrated_confidence,
                 wr_verdict_basis) = row
                ...
                if state:
                    ...
                    if (verdict in ('REFUTED', 'PARTIALLY REFUTED', 'REFUTED_SETUP')
                        ...
                        try:
                            _rf_throttled = should_throttle(conn, source="refutation_followup")
                        except Exception:
                            _rf_throttled = False
                        if _rf_throttled:
                    ...
                    if (verdict in ('SUPPORTED', 'CONFIRMED')
                            ...
                            and inserted < 2):
                        try:
                            _cf_throttled = should_throttle(conn, source=None)
                        except Exception:
                            _cf_throttled = False
                    ...
                    if inserted:
                        print(f"  Inserted {inserted} curiosities into DB")

                    state["last_updated"] = now_unix

                # 3. Mark as applied
                conn.execute(
                    "UPDATE worker_results SET applied = 1 WHERE id = ?", (result_id,)
                )
                applied += 1
                print(f"  Applied: {exp_id} — {key_finding[:60] if key_finding else 'no finding'}")
                ...
                if task_id:
                    update_completed(task_id, result_id, domain, conn=conn)
        ...
        print(f"\nApplied {applied} worker results")
