...
~82 cron jobs: ~79 script-only (fixed schedule) + 3 LLM-driven (Director, inspector, external-probe — state injected via preprocessor scripts). Deep documentation lives in the sections above; this table is the inventory.
...
| `reclassify_domains.py` | 15m | Reclassify uncategorized experiments |
...
**The de-fork migration EXECUTED 2026-07-08** (plan: `docs/defork-plan.md`; execution + verification: changelog), and the base was **updated to upstream tip `56a8e81d3` the same day**: branch `prometheus-fork` = origin/main + **17 commits mapping 1:1 to the open upstream PRs** (#61221-35; rollback tags `pre-defork-cutover`, `pre-update-20260708`). The base tree carries **only the PR bucket + a deployment-residue commit**; all site policy lives in an update-proof layer outside the repo. `background_review.py`, `skills_tool.py`, and `cron/jobs.py` are byte-identical to upstream; `prompt_builder.py` (KANBAN_GUIDANCE wording, PR #61222) and `process_registry.py` (venv-env, PR #61221) carry their PR hunks. Update procedure: cherry-pick the PR branches onto new tip + residue commit — NOT a history replay; and diff every cherry-pick against tip treating net deletions as suspect (a stale-base patch once silently reverted an upstream leak fix).
