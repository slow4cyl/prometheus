...
- `worker_results.finding_hash` is a RETIRED legacy column: pre-defork fleet workers wrote ad-hoc hashes into it (mixed sha256/md5/truncated formats); no script reads or writes it. Kept for column-order stability on the live DB; do not revive or backfill.
