...
- Show HN: My research agent predicts its own claim-transfer at 58%. Publishing that is the point

**Body:**

I built an autonomous research system. Before the architecture, here is what it measured about itself:

- Asked to predict which of its own claims would transfer to new domains, it scored 58% (on ~500 self-probes; 53% at the first N=32 measurement), barely above chance. It responded by hair-cutting transfer confidence across the board.
...
- A calibration audit found it over-trusts MONOTONIC-shaped claims 68.7% of the time. It reweighted them.

The system runs continuously on one consumer gaming PC with an RTX 5090. It generates its own questions, runs experiments with preserved code (~133k experiments, ~77k claims at snapshot), and spends a large share of its compute attacking its own conclusions: adversarial replication, cross-domain disconfirmation gates, novelty checks against actual literature indexes, independence and circularity gates.
...
Asked to predict which of its own claims transfer to new domains, it scored 58% (~500 self-probes; 53% at N=32). Barely above chance. It cut its own confidence in response.
...
**5/**
It also audits its own calibration by claim shape. Finding: MONOTONIC-type claims ("more X, more Y") were over-trusted 68.7% of the time, so it reweighted them.

Contradicted claims aren't deleted either. They get routed to an attack lane and fought over.

**6/**
Context: this is my first project. No CS degree. About a month, one consumer gaming PC with an RTX 5090. ~131k experiments, ~77k claims, 85 cron jobs, a local vLLM worker fleet plus an API lane. Built on the open-source hermes-agent substrate.
